...
type RequestVoteReply struct {
	// Your data here (2A).
	Term        int // 这个不是很明白 为什么这里需要update term
	VoteGranted bool
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	rf.mu.Lock()
	defer rf.mu.Unlock()
	// 这里收到了请求 所以重置elect timeout
	// rf.heartsbeats <- true
	DPrintf("[%d] received the voteRequest from [%d]\n", rf.me, args.CandidateId)

	// 如果选过了
	if rf.lastVoteTerm >= args.Term {
	...
	// 	return
	// }
	// 1.如果是leader 啥也不说了 改变对方term
	// 这里就不比较term了 因为可能网络中间断开导致其term 变得很大其实没有什么用
	// 如果自己是leader 返回term 使candidate更新
	if rf.rule == Leader {
		reply.Term = rf.curTerm
		reply.VoteGranted = false
		return
	}
	// 2.是candidate 不能给对手投票吧
	if rf.rule == Candidate {
		reply.VoteGranted = false // candidate怎么可能给平级之人投票呢
		return
	}

	// 3.是follower 想要票也有要求 对应于 election striction
	// TODO: 2(B)
	...
	DPrintf("[%d] lastLogIndex[%d] lastLogTerm[%d]\n", args.CandidateId, args.LastLogIndex, args.LastLogTerm)
	if rf.curTerm > args.Term || args.LastLogTerm < selfLastLogTerm || args.LastLogIndex < selfLastLogIndex {
		reply.VoteGranted = false
		return
	}
	if rf.lastVoteTerm == -1 {
		DPrintf("[%d] vote for [%d] as follower\n", rf.me, args.CandidateId)
		reply.VoteGranted = true
		rf.votedFor = args.CandidateId
		rf.lastVoteTerm = args.Term
		return
...
func (rf *Raft) AttemptElection() {
	rf.mu.Lock()
	rf.curTerm++
	if rf.lastVoteTerm >= rf.curTerm {
		// 说明投过票了 已经失去了成为candidate的资格
		rf.mu.Unlock()
		return
	}
	...
	for server := range rf.peers {
		...
		go func(server int) {
			...
			// log.Printf("[%d] sending election request to [%d]", rf.me, server)
...
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		// time.Sleep().
		rf.mu.Lock()
		// DPrintf("[%d] rule: %d", rf.me, rf.rule)
		selfRule := rf.rule
		rf.mu.Unlock()
		if selfRule == Leader {
			select {
			...
			for server := range rf.peers {
				...
				go func(server int) {
					...
					personalArgs := args
					personalArgs.PrevLogIndex = rf.nextIndex[server] - 1 // matchIndex和nextIndex-1有什么区别
					...
					if reply.Success {
						...
						for i := range rf.peers {
							if i == rf.me {
								allMatchIndex = append(allMatchIndex, rf.commitIndex)
							} else {
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	for server := range rf.peers {
		...
		go func(server int) {
			...
			if reply.NoAction {
				DPrintf("[%d]NoAction  in Start", server)
				return
			}
...
func (rf *Raft) AppendEntity(args *AppendEntityArgs, reply *AppendEntityReply) {
	...
	if rf.commitIndex < args.LeaderCommit {
		// 需要更新了
		old_commitIndex := rf.commitIndex
		DPrintf("need to update commitIndex oldIndex[%d]  len(log)[%d] leaderIndex[%d]\n", old_commitIndex, len(rf.log), args.LeaderCommit)
		rf.commitIndex = Min(args.LeaderCommit, len(rf.log)-1)
		// 可能有很多需要commit 和 apply
		for i := old_commitIndex + 1; i <= rf.commitIndex; i++ {
			DPrintf("||||||||||||||  server[%d] apply[%d] ||||||||", rf.me, i)
	...
	DPrintf("[%d] receivede the log [%v]\n", rf.me, args.Entries)
	reply.Success = true
	reply.NoAction = false
	reply.Term = 100
	// DPrintf("%v\n", reply)
	// relpy.term到底是什么  TODO:
}
